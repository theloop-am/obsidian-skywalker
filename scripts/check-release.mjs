 ...
 * file and the changelog. A theme has no versions.json - that map is a plugin's
 * way of naming the Obsidian version each release needs. With --notes it prints that
...
async function main() {
  ...
  else console.log(`Release: ${version} agrees across the manifest, the package file and the changelog.`);
}
